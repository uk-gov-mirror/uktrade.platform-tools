locals {
  ...
  cache_invalidation_assumed_roles = setunion([for i, pipeline in var.var.pipelines : toset([for env in pipeline.environments : "arn:aws:iam::${local.base_env_config[env.name].dns_account}:role/${var.application}-${env.name}-pipeline-deployment-role"])])
