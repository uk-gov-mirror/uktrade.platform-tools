...
from dbt_platform_helper.entities.service import ScheduledJobConfig
...
class ServiceManager:
    ...
    def get_service_models(self, application, environment, services=None) -> list[ServiceConfig]:
        ...
        for service in services:
            ...
            if merged_config.get("type", "") == "Scheduled Job":
                service_model = ScheduledJobConfig(**merged_config)
            else:
    ...
    def migrate_copilot_manifests(self) -> None:
        ...
        for dirname, _, filenames in os.walk("copilot"):
            if "manifest.yml" in filenames and "environments" not in dirname:
                ...
                if service_manifest["type"] == "Scheduled Job":
                    service_manifest = self._migrate_scheduled_job_manifest(service_manifest)
                    ...
                    message = (
                        f"# Generated by platform-helper "
                        f"{self.installed_version_provider.get_semantic_version(PLATFORM_HELPER_PACKAGE_NAME)} "
                        f"/ {current_date}.\n\n"
                    )

                    self.file_provider.write(
                        f"{service_path}/service-config.yml",
                        dict(service_manifest),
                        message,
                    )
    ...
    def _migrate_scheduled_job_manifest(self, service_manifest: OrderedDict) -> OrderedDict:
        """Transform a Copilot Scheduled Job manifest into a platform service-
        config.yml structure."""

        on_block = service_manifest.pop("on", {})
        if "schedule" in on_block:
            service_manifest["schedule"] = on_block["schedule"]

        if "network" in service_manifest:
            del service_manifest["network"]
        if "observability" in service_manifest:
            del service_manifest["observability"]
        if "env_file" in service_manifest:
            del service_manifest["env_file"]

        if "image" in service_manifest:
            if "location" in service_manifest["image"]:
                image_without_tag = service_manifest["image"]["location"].rsplit(":", 1)[0]
                service_manifest["image"]["location"] = image_without_tag
            elif "build" in service_manifest["image"]:
                del service_manifest["image"]["build"]
        ...
        return self.file_provider.remove_empty_keys(service_manifest)
